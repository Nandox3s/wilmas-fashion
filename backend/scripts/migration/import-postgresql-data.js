...
const date = (value) => {
  const normalized = typeof value === 'number' ? new Date(value) : new Date(String(value).replace(' ', 'T').replace(/(?<!Z)$/, 'Z'))
  if (Number.isNaN(normalized.getTime())) throw new Error('Invalid migration timestamp')
  return normalized
}
const safe = async (type, id, operation) => { try { await operation() } catch (error) { const message = String(error.message).split('\n').map((line) => line.trim()).filter(Boolean).at(-1) || 'Import operation failed'; rejected.push({ type, id, code: error.code || 'ERROR', message }) } }
for (const row of data.users) await safe('user', row.id, () => prisma.user.upsert({ where: { email: String(row.email).trim().toLowerCase() }, update: { name: row.name, password: row.password, role: row.role === 'ADMIN' ? 'ADMIN' : 'USER' }, create: { id: row.id, name: row.name, email: String(row.email).trim().toLowerCase(), password: row.password, role: row.role === 'ADMIN' ? 'ADMIN' : 'USER', createdAt: date(row.createdAt), updatedAt: date(row.updatedAt) } }))
for (const row of data.products) await safe('product', row.id, () => prisma.product.upsert({ where: { sku: String(row.sku).trim().toUpperCase() }, update: { name: row.name, brand: row.brand, category: row.category, sizes: JSON.parse(row.sizes || '[]'), color: row.color, price: Number(row.price).toFixed(2), discount: Number(row.discount || 0).toFixed(2), onOffer: Boolean(row.onOffer), stock: row.stock, image: row.image }, create: { id: row.id, name: row.name, sku: String(row.sku).trim().toUpperCase(), brand: row.brand, category: row.category, sizes: JSON.parse(row.sizes || '[]'), color: row.color, price: Number(row.price).toFixed(2), discount: Number(row.discount || 0).toFixed(2), onOffer: Boolean(row.onOffer), stock: row.stock, image: row.image, createdAt: date(row.createdAt), updatedAt: date(row.updatedAt) } }))
for (const row of data.sales) await safe('sale', row.id, () => prisma.sale.upsert({ where: { id: row.id }, update: { userId: row.userId, productId: row.productId, quantity: row.quantity, total: Number(row.total).toFixed(2) }, create: { id: row.id, userId: row.userId, productId: row.productId, quantity: row.quantity, total: Number(row.total).toFixed(2), createdAt: date(row.createdAt) } }))

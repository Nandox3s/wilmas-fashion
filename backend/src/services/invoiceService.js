...
export class InvoiceService {
  ...
  authorize(invoice, user) { if (user.role !== 'ADMIN' && invoice.order.userId !== user.id) throw new HttpError(403, 'No tienes permisos para ver esta factura.', 'FORBIDDEN') }
  async get(id, user) { const row = await this.prisma.invoice.findUnique({ where: { id: Number(id) }, include: { order: true, events: true } }); if (!row) throw new HttpError(404, 'Factura no encontrada.', 'INVOICE_NOT_FOUND'); this.authorize(row, user); return serializeMoney(row) }
  async byOrder(reference, user) { const row = await this.prisma.invoice.findFirst({ where: { order: { reference: String(reference) } }, include: { order: true, events: true } }); if (!row) throw new HttpError(404, 'Factura no encontrada.', 'INVOICE_NOT_FOUND'); this.authorize(row, user); return serializeMoney(row) }
  async process(id) {
  ...
  async orderDocument(orderId, type, user) {
    ...
    if (!key) throw new HttpError(404, 'El archivo de la factura no está disponible.', 'INVOICE_FILE_NOT_AVAILABLE')
